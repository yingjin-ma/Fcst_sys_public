...
gau_dft  =["BLYP","LC-BLYP","B3LYP","CAM-B3LYP","M06","M06-2x","wb97xd","bhandhlyp","PBE1PBE"]
gau_basis=["6-31g","6-31g*","6-31+g*"]
gau_keywords=[]
for idft in gau_dft:
   for ibasis in gau_basis:
      gau_keywords.append("#p "+idft+"/"+ibasis+" pop=full nosym") 
...
print (" ")
for gau_keyword in gau_keywords:
   print ("Generated Gaussian inputs : ",gau_run,gau_cores,gau_mem,gau_keyword)
...
"Not done due to no SDFs only Gaussian"
exit(0)

#moldirs.append("/home/yingjin/Softwares/Fcst_sys/16-2-2_1to25_M062x-631g/sdf_only_valid_atoms_working5")
...
#                 exit()
                  ...
                  os.system("sed -i \'4 i "+gau_keyword+"\' "+output2)
...
#time_stamp_folder="result_"+time.strftime("%Y-%m-%d-%H-%M-%S",time.localtime())
      stamp_folder =moldir + "/Gaussian_" + gau_keyword.repalce("/","")
