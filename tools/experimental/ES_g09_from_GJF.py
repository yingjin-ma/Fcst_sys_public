...
gjf_folders.append("/work1/scykd/mayj/Fcst_sys/16-2-2_1to25_M062x-631g/ring_sub")


for newpar in gau_keywords:
       
   newpar_folder="Gaussian-"+newpar.replace("/","_").replace("*","s").replace("+","p") + "_TD"
...
#                        print(str(lines))
                        for line in lines:
                           imatch_chk=line.find("chk")
                           if imatch_chk != -1 :
                              continue
                           else:
                              newline=line.replace(oldpar,newpar+" TD")
                              fw.write(newline)
