...
class RfTool(ModelTool):
    ...
    def __init__(self,chemspace,config,suits2,folder_sdf,folder_mod,target):
        '''
        construction
        chemspace
        ...
        if os.path.exists(folder_mod+"/RFC.m"):
            self.clf = joblib.load(folder_mod+"/RFC.m")
    ...
    def eval(self,path=None,modeldir='rfmodel',chemspace="B3LYP_6-31g",write=False):
        ...
        if write is True:
            if tra_size<4000:
                ...
                wbdir="results/"
            wbname=wbdir+'tes_rf_'+self.chemspace+'.xlsx'
