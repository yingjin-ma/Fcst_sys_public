...
class MGCNModel(nn.Module):
    ...
    def __init__(self,
                 dim=32,  # original 128
                 output_dim=6, # original 12
                 edge_dim=32, # original 128
                 cutoff=5.0,
                 width=1,
                 n_conv=5,
                 norm=True,
    ...
    def forward(self, g,basisnum,basisnums):
        ...
        if self.norm:
            g.ndata["res"] = g.ndata["res"] * self.std_per_node + self.mean_per_node

        mgcnout = dgl.sum_nodes(g, "res")
        basisnum = basisnum.squeeze(1)
        # out1=th.cat((mgcnout,basisnum),1)
        # out=th.cat((out1,basisnums),1)
        out = th.cat((mgcnout, basisnums), 1)
        out=self.bn1(out)
        res=self.fclayer1(out)
        #res=self.bn(res)
        res=F.tanh(res)
        res=self.fclayer2(res)
        res=self.fclayer3(res).squeeze(-1)
        return res
